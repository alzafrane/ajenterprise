...
export default function Testimonials() {
  return (
    <section id="testimonials" className="py-20 bg-gray-100 border-t border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-primary font-semibold tracking-wide uppercase text-sm mb-2">Testimonials</h2>
          <h3 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Happy Customers in Ahmedabad</h3>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {testimonials.map((t, i) => (
            <div key={i} className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 relative hover:shadow-md transition-shadow">
              <Quote className="absolute top-8 right-8 text-gray-100/50" size={64} fill="currentColor" />
              <div className="flex gap-1 text-yellow-400 mb-6">
                {[...Array(5)].map((_, idx) => (
                  <Star key={idx} fill="currentColor" size={18} />
                ))}
              </div>
              <p className="text-gray-700 mb-8 relative z-10 italic leading-relaxed">"{t.text}"</p>
